...
for epoch in range(cur_epoch, cfg['max_epoch']):
    for i, data in enumerate(train_loader):  # inner loop within one epoch
        ...
        # if i % 1 == 0:
            ...
            is_fg = ['mask', 'lab', 'lab_crop', 'fg_mask', 'bg_mask', 'fg_mask_crop', 'mask_crop', 'ints_mask_crop', 'fg_g_mask', 'fg_l_mask', 'comb_mask']
            for name, img in results.items():
                no_fg = True
                if name in is_fg:
                    no_fg = False
                save_name = 'epoch-%d_iter-%d_%s_'%(epoch, i, name)+'.png'
                if name == 'lab' or name == 'lab_crop':
                    lab = lab2im(img)
    ...
    if epoch % cfg['test_freq'] == 0:
        with torch.no_grad():
            ...
            for i, data in enumerate(test_loader):
                ...
                trainer.forward(train=False)
                results, img_name = trainer.visual_results()
                cv2.imwrite(os.path.join(cur_save_dir, img_name[0]+'.png'), tensor2im(results['masked_fake']))
