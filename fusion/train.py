...
parser.add_argument('--resume_dir', type=str, default='outputs/cityscapes512x256/2022-01-11_00-49-45/', help="outputs path")
...
for epoch in range(cur_epoch, cfg['max_epoch']):
    ...
    for i, data in enumerate(pbar):  # inner loop within one epoch
        ...
        # if i % 1 == 0:
            ...
            is_fg = ['mask', 'lab', 'lab_crop', 'fg_mask', 'bg_mask', 'bg_mask_ori', 'edge_map', 
                    'car_g_mask', 'person_g_mask', 'comb_mask', 
                    'car_inst_mask_filtering', 'car_mask_filtering', 
                    'person_inst_mask_filtering', 'person_mask_filtering']
            for name, img in results.items():
                ...
                elif '_colormap' in name:
                    cv2.imwrite(os.path.join(cur_img_dir, save_name), img[0].cpu().float().numpy().astype(np.uint8))
    ...
    if epoch % cfg['test_freq'] == 0:
        with torch.no_grad():
            ...
            for i, data in enumerate(test_loader):
                ...
                cv2.imwrite(os.path.join(cur_save_dir, img_name[0]+'.png'), tensor2im(results['masked_F']))
