...
class SIE_Dataset(Dataset):
    def __init__(self, cfg, dataset_root, split='train', mask_type='ff_mask', dataset_name=''):
        ...
        if self.split == 'test':
            if self.mask_type == 'addition' or self.mask_type == 'addition_new':
                self.dir_img = os.path.join(dataset_root, mask_type, 'images')
                self.dir_lab = os.path.join(dataset_root, mask_type, 'labels')
                self.mask_path = os.path.join(dataset_root, mask_type, 'mask')
                self.dir_ins = os.path.join(dataset_root, mask_type, 'inst_map')
            elif self.mask_type == 'removal':
                self.dir_img = os.path.join(dataset_root, mask_type, 'images')
                self.dir_lab = os.path.join(dataset_root, mask_type, 'labels')
                self.mask_path = os.path.join(dataset_root, mask_type, 'mask')
                self.dir_ins = os.path.join(dataset_root, mask_type, 'labels')
            elif self.mask_type == 'dir':
                self.dir_img = os.path.join(dataset_root, 'images')
                self.dir_lab = os.path.join(dataset_root, 'labels')
                self.mask_path = os.path.join(dataset_root, 'mask')
                self.dir_ins = os.path.join(dataset_root, 'inst_map')
            elif self.mask_type == 'out_dis':
                self.dir_img = os.path.join(dataset_root, 'images')
                self.dir_lab = os.path.join(dataset_root, 'labels')
                self.mask_path = os.path.join(dataset_root, 'mask')
                self.dir_ins = os.path.join(dataset_root, 'inst_map')
            elif self.mask_type == 'test_all':
                self.dir_img = os.path.join(dataset_root, mask_type, 'images')
                self.dir_lab = os.path.join(dataset_root, mask_type, 'labels')
                self.dir_ins = os.path.join(dataset_root, mask_type, 'inst_map')
            elif self.mask_type == 'train_all':
                self.dir_img = os.path.join(dataset_root, 'train', 'images')
                self.dir_lab = os.path.join(dataset_root, 'train', 'labels')
                self.dir_ins = os.path.join(dataset_root, 'train', 'inst_map')
            else:
        ...
        self.name_list = os.listdir(self.dir_img)
        self.name_list.sort()
        self.name_list = [n[:-4] for n in self.name_list if n.endswith('jpg')]
    ...
    def __getitem__(self, index):
        # object images
        NUM_OBJ_PER_IMG = 4
        car_sty_images = torch.zeros((NUM_OBJ_PER_IMG, 3, 64, 64))
        car_sty_inst_images = torch.zeros((NUM_OBJ_PER_IMG, 1, 64, 64))
        for idxobj in range(NUM_OBJ_PER_IMG):
            ss = random.randint(0, self.car_num-1)

            sty_img = cv2.imread(os.path.join(self.car_dir, self.car_sty_list[ss]))
            car_sty_images[idxobj] = get_transform(sty_img)

            sty_inst_mask = cv2.imread(os.path.join(self.car_inst_dir, self.car_sty_list[ss][:-4]+'.png'), 0) / 255
            sty_inst_mask = get_transform(sty_inst_mask, normalize=False)
            car_sty_inst_images[idxobj] = sty_inst_mask.float()

        person_sty_images = torch.zeros((NUM_OBJ_PER_IMG, 3, 64, 64))
        person_sty_inst_images = torch.zeros((NUM_OBJ_PER_IMG, 1, 64, 64))
        for idxobj in range(NUM_OBJ_PER_IMG):
            ss = random.randint(0, self.person_num-1)

            sty_img = cv2.resize(cv2.imread(os.path.join(self.person_dir, self.person_sty_list[ss])), (64, 64))
            sty_img = get_transform(sty_img)
            person_sty_images[idxobj] = sty_img

            sty_inst_mask = cv2.imread(os.path.join(self.person_inst_dir, self.person_sty_list[ss][:-4]+'.png'), 0) / 255
            sty_inst_mask = cv2.resize(sty_inst_mask, (64, 64), interpolation=cv2.INTER_NEAREST)
            sty_inst_mask = get_transform(sty_inst_mask, normalize=False)
            person_sty_inst_images[idxobj] = sty_inst_mask.float()
        ...
        self.real_h, self.real_w = lab.shape[0], lab.shape[1]
        # print(self.real_h, self.real_w)     # 256 512
        ...
        if self.dataset_name == 'cityscapes512x256':
            fg_class_list = [24, 26]  # car, person
        ...
        if self.split == 'train':
            ...
            if self.mask_type == 'addition' or self.mask_type == 'addition_new':
                mask = cv2.imread(os.path.join(self.mask_path, name + '.png'), 0) / 255
                mask = mask.reshape((1,) + mask.shape).astype(np.float32)
            elif self.mask_type == 'removal':
                mask = cv2.imread(os.path.join(self.mask_path, name + '.png'), 0) / 255
                mask = mask.reshape((1,) + mask.shape).astype(np.float32)
            elif self.mask_type == 'dir':
                mask = cv2.imread(os.path.join(self.mask_path, name + '.png'), 0)
                mask = mask.reshape((1,) + mask.shape).astype(np.float32)
            elif self.mask_type == 'out_dis':
                mask = cv2.imread(os.path.join(self.mask_path, name + '.png'), 0) / 255
                mask = mask.reshape((1,) + mask.shape).astype(np.float32)
            elif self.mask_type == 'test_all' or self.mask_type == 'train_all':
        ...
        if self.dataset_name == 'cityscapes512x256':
                ...
                if np.sum(curr_iid) > 0:  # iid in masked region
                    ...
                    if max_class in fg_class_list:
                        ...
                        if np.sum(curr_iid_inv) > 0:  # iid also appear in non-masked region
                            # curr_mask = curr_iid+curr_iid_inv
                            if mask_type != 1000000000000:
                                fg_inp_inst.append(curr_inst_mask)  # curr_mask
            ...
            person_gt_crop, person_gt_crop2048, person_masked_img_crop, person_lab_crop, person_ints_mask_crop, person_mask_crop, person_inst_cors, person_g_mask_inst \
                = self.inst_crop(img, gt2048, masked_img, lab, mask, fg_inst, fg_classes, max_num=self.max_num, 
                                 class_id=24, min_size=(0,0), crop_size=(64,64), sum_min_ratio=0.35, ratio_range=None)
        ...
        return {'img': img, 'masked_img': masked_img, 'gt2048': gt2048, 'lab': lab, 'mask': mask, 
                ...
                'inst_map': inst_map,
                'car_sty_images': car_sty_images, 'car_sty_inst_images': car_sty_inst_images,  'person_sty_images': person_sty_images,  'person_sty_inst_images': person_sty_inst_images,
                'name': name}
    ...
    def select_inst_cor(self, inst_mask_list, class_list, mask, class_id, min_size, sum_min_ratio, ratio_range):
        ...
        for i, ints in enumerate(inst_mask_list):
            ...
            if class_list[i] == class_id:
                ...
                # 训练 F 时，np.sum(ints_masked) / np.sum(ints) > 0.4 （改为 0）
                if sum(cor) > 0 and np.sum(ints_masked) / np.sum(ints) > 0 and sum_ratio > sum_min_ratio:
                    if r_curr > ratio_range[0] and r_curr < ratio_range[1]:
                        g_mask_inst.append(ints)
                        cors_list.append(cor)
            else:
                pass
                # print('error inst class')
